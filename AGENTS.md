...
├── local_server/           ──── PART 3d: RUST LOCAL SERVER (单二进制局域网部署，2026-08) ────
│   ├── src/ocr/           # CRNN 推理（onnxruntime/ort + INTER_AREA 预处理 + trie 解码）
│   ├── src/{api,service,db,models,export}.rs  # axum API + SQLite（/api/v1 契约与 Kotlin 云端一致）
│   ├── tests/             # api_contract.rs（15 测试 = Kotlin 9 个 MockMvc 移植）+ parity.rs
│   ├── src/bin/bench_acceptance.rs  # Rust 端验收门禁（4 真实集参照值硬编码）
│   ├── build-release.bat  # 构建自包含 release/（exe + onnxruntime.dll + data/ + models/）
│   └── start-local.bat    # 一键启动（无 JDK/Python/Node 依赖）
...
### Rust 本地方案（local_server/，2026-08-14 起可用）

**定位**：双后端并存——Kotlin Spring（云端，PostgreSQL + Python image_service）与 Rust（本地局域网单二进制）共用同一前端与 /api/v1 契约。前端零改动（同源相对路径 /api/v1）。

```bash
# 构建发布版（先 cd ../frontend && npm run build，产物 embed 进 exe）
cmd //c build-release.bat        # 生成 release/（exe + onnxruntime.dll + data/ + models/ 自包含）
# 一键启动（发布/开发机均可）：
cd release && start-local.bat    # 或双击；默认 :8080，局域网 http://<本机IP>:8080
...
ORT_DYLIB_PATH=E:\devtools\conda\envs\bead-train\Lib\site-packages\onnxruntime\capi\onnxruntime.dll \
  cargo run    # 默认 :8080；env 覆盖：BEAD_PORT/BEAD_DB_PATH/BEAD_UPLOADS_DIR/BEAD_ARTIFACT_DIR

# 测试（19 个：3 unit + 15 contract + 1 parity）：
ORT_DYLIB_PATH=... cargo test
# Rust 端验收门禁（4 真实集，指标须与 Python 参照一致，参照值硬编码在 bin）：
ORT_DYLIB_PATH=... cargo run --release --bin bench_acceptance
```

**关键环境坑**（已固化 local_server/.cargo/config.toml）：MinGW link 抢占 MSVC → linker 写全路径；Windows SDK 26100 补装；LIB 显式设置。ort 用 load-dynamic + api-23（与 Python onnxruntime 1.23.2 同核心）。数值一致三定律：batch 统一 128、输入 uint8 量化、INTER_AREA=加权区域平均。详见 daily 记忆 2026-08-13/14。
