...
拼豆助手是一款**AI拼豆图纸识别工具**，它会自动识别每个格子里的编码（如 H7、F2、C21），对照品牌颜色库，生成一份清晰、可读的电子图纸，当前仅支持 mard 品牌的豆子颜色编码。
...
- **启动**：双击 `start-local.bat` —— 后台启动服务，并自动打开浏览器页面
  - 打开的地址是**本机的局域网地址**（形如 `http://192.168.x.x:5173`），**同一局域网内的其他设备（手机/平板/其他电脑）也可以直接访问这个地址**使用拼豆助手
...
> 💌 **帮助改进**：如果你在使用中发现识别错误，请在「校正」页修改后**导出校正数据**，发送到我的邮箱 **pseriaml@163.com**。每一个校正样本都能帮助我改进识别模型，让拼豆助手越来越聪明，先谢过大家！
